...
static inline bool of_clk_is_ignore_pm(struct device_node *np, const char *name)
{
	return of_property_match_string(np, "ignore-pm-clocks", name) >= 0;
}

static inline bool of_clk_is_invalid(struct device_node *np, const char *name)
{
...
static inline struct cc_desc *devm_cc_alloc(struct device *dev, int max_clk)
{
	...
	ccd->data.clks =
		devm_kcalloc(dev, max_clk, sizeof(*ccd->data.clks), GFP_KERNEL);
	if (!ccd->data.clks)
		return NULL;
#ifdef CONFIG_PM
	ccd->pm_data =
		devm_kcalloc(dev, max_clk, sizeof(*ccd->pm_data), GFP_KERNEL);
	if (!ccd->pm_data)
		return NULL;
#endif
	return ccd;
