	...
	if (check_status == status) {
		if (count > 1) {
			pr_err("[%s] (%u) %s irq status is not change"
			       ...
			       DEV_NAME, irq, mux_data->index ? "ISO" : "MISC",
			       status, enable);
		} else {
			count++;
		}
...
static void __init mux_init_each(struct irq_mux_data *mux_data,
				 void __iomem *base, u32 irq, u32 status,
				 u32 enable, int nr)
{
	mux_data->base = base;
	mux_data->index = nr;
			      ...
			      struct device_node *parent)
{
	 ...
	 */
	rtk_domain = irq_domain_add_simple(np, (nr_irq * IRQ_INMUX), 160,
